<template>
	<view class="caigou_index">
		<uni-nav-bar left-icon="back" :statu-bar="navBar"  title="修改密码" :status-bar="navBar" color="#1A1A1A" @clickLeft="goPage"></uni-nav-bar>
	...
	export default {
		...
		methods: {
			...
			submit() {
				...
				rs.postRequests('vehicleModifyPassword', params, res => {
					let data = res.data;
					if (data.code == 200) {
						rs.Toast('修改成功');
						setTimeout(function() {
