<template>
	<view class="delivery">
		...
		<view class="top_search">
			...
			<view style="height:10rpx;background:  #F9F9FB;"></view>
		</view>
		<view class="all_Order" v-if="bitmap">
			...
			<view v-for="(item,index) in itemList" class="sign_good" @click="detailPage(item.id)" :class="index==itemList.length-1?'':'border_bottom'">
				...
				<view>
					...
					<view class="height"><text>收货手机 :</text><text>{{item.mobile}}</text><text class="iconfont icondianhua1" @click.stop="callPhone(item.mobile)"></text></view>
	...
	export default {
		...
		methods: {
			...
			requestItemList() {
				...
				rs.getRequests("salesmanList", data, (res) => {
