<template>
	<view class="caigou_index">
		...
		<view class="fix_top">
			<uni-nav-bar left-icon="back"  title="我要收款" color="#1A1A1A" @clickLeft="goPage"></uni-nav-bar>
			...
			<view class="top">
				待收款：<text class="red_font">¥{{total}}</text>
			</view>
		</view>

		<view style="height: 270rpx;background: #F9F9F9;"></view>

		<view class="bill" v-if="bitmap">

			<view class="item" v-for="item in list">
				<view class="flex flex_align_center">{{item.contact}}<text style="height:28rpx;width:1px;background: #d5d5d5;display: inline-block;margin:0 15rpx;"></text>{{item.nickname}}</view>
				<view class="red_font">¥{{item.total_sales}}</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			requestBill() {
				...
				rs.getRequests("salesmanGathering", data, (res) => {
