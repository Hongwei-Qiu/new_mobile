<template>
	<view class="detail">
		<uni-nav-bar left-icon="back" :statu-bar="navBar"  title="供应单详情"  color="#1A1A1A" @clickLeft="goPage"></uni-nav-bar>
		...
		<view class="list_box">
			<view class="top_item">
				...
				<view class="txt">
					...
					<view v-if="listDetail.type==3">现场采购</view>
				</view>
				<view class="txt">
					<view>状态</view>
	...
	export default {
		...
		methods: {
			...
			requestListDetail() {
				...
				rs.getRequests("supplierListDetail", data, (res) => {
			...
			save(item, index) {
				...
				rs.postRequests("supplierPurchaseEntry", data, (res) => {
					 let {data}=res;
					if(data.code==200){
						rs.Toast('保存成功');
