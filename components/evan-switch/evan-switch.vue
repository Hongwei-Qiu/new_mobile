<template>
	<view @click="toggle" class="evan-switch" :class="{'evan-switch--disabled':disabled}" :style="{width:2*size+'px',height:switchHeight,borderRadius:size+'px',backgroundColor:currentValue===activeValue?activeColor:inactiveColor}">
		...
		:style="{'color':currentValue?activeColor:'#c2c2c2',width:size+'px',height:size+'px','text-align':'center','line-height':size+'px',transform:currentValue===activeValue?`translateX(${size}px)`:`translateX(0)`}">
		{{currentValue?'开':'关'}}</view>
	</view>
</template>
